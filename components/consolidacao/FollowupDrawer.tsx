...
import { X, Save, Loader2, Phone, MessageSquare, Home, CalendarCheck, BookOpen, FileText, CheckCircle2, Calendar } from 'lucide-react'
import {
  FollowupEnriched,
  FollowupStatus,
    ...
    >
      ...
      >
        ...
        <div className="flex-1 px-6 py-5 space-y-6 overflow-y-auto">
          ...
          {/* Próximo Revisão de Vidas */}
          <section>
            <SectionHeader icon={BookOpen} label="O Revisão de Vidas" />
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-semibold text-slate-500 mb-1.5">Evento</label>
                <CustomSelect
                  value={form.next_review_event_id as string}
                  onChange={(v) => set('next_review_event_id', v)}
                  options={[
                    { value: '', label: 'Nenhum' },
                    ...reviewEvents.map((ev) => ({
                      value: ev.id,
                      label: `${ev.name} — ${new Date(ev.start_date).toLocaleDateString('pt-BR')}`,
                    })),
                  ]}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-500 mb-1.5">Data prevista</label>
                <DatePickerInput
                  value={form.next_review_date as string}
                  onChange={(v) => set('next_review_date', v)}
                  placeholder="dd/mm/aaaa"
                />
              </div>
            </div>
