...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  if (!publicada || publicada.status !== 'publicada') {
    return NextResponse.json({ error: 'A escala precisa estar publicada para enviar disparos.' }, { status: 400 })
  }

  const slots: SlotResult[] = (publicada.dados as any)?.slots ?? []
  if (slots.length === 0) {
    return NextResponse.json({ error: 'Nenhum slot encontrado na escala publicada.' }, { status: 400 })
  }

  // ── Voluntários com telefone ──────────────────────────────────────────────
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // TIPO: mes — um disparo por voluntário com a lista completa de alocações
  // ──────────────────────────────────────────────────────────────────────────
  if (tipo === 'mes') {
    // Agrupa por person_id
    ...
    if (entries.length === 0) {
      return NextResponse.json({ ok: true, enviados: 0, aviso: 'Nenhum voluntário com alocação.' })
    }

    // No teste envia só para o primeiro voluntário usando phone_teste
    const targets = teste ? [entries[0]] : entries

    for (const [personId, assignments] of targets) {
      const person = personMap[personId]
      const phone  = teste ? phone_teste : person?.phone
      if (!phone) {
        resultados.push({ person_id: personId, nome: person?.full_name ?? personId, phone_last4: '????', success: false })
        continue
      }

      const nomeExib = getNomeExibicao((teste && nome_teste) ? nome_teste : (person?.full_name ?? ''))
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // TIPO: lembrete_3dias / lembrete_1dia / dia_da_escala — um disparo por alocação do dia
  // ──────────────────────────────────────────────────────────────────────────
  if (tipo === 'lembrete_3dias' || tipo === 'lembrete_1dia' || tipo === 'dia_da_escala') {
    ...
    const targetSlots = slotsForDate
    let testAssignment: { slot: SlotResult, assignment: Assignment } | null = null

    // Se for teste, procura o primeiro voluntário preenchido em qualquer slot para simular
    if (teste) {
      ...
      // Se não houver ninguém escalado ainda nem no rascunho de banco, usamos dados fictícios para não travar o botão de teste
      if (!testAssignment) {
        testAssignment = {
    ...
    if (!teste && slotsForDate.length === 0) {
      return NextResponse.json({
        ok: true, enviados: 0,
        aviso: `Nenhum culto/evento encontrado para ${formatDateFullBr(targetDate)}. Verifique se a data está correta.`,
      })
    ...
    outer: for (const slot of processSlots) {
      ...
      for (const a of assignments) {
        ...
        const nomeExib = getNomeExibicao((teste && nome_teste) ? nome_teste : (person?.full_name ?? a.person_name))
  ...
  return NextResponse.json({ ok: true, enviados, erros, resultados })
}
