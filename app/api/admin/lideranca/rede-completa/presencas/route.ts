...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Busca TODA a árvore de liderança (recursiva)
    let leadershipTree: Array<{ id: string }> = []
    try {
      leadershipTree = await getLeadershipTree(personId)
    } catch (treeErr) {
      console.error('rede-completa/presencas: erro ao buscar árvore de liderança:', treeErr)
      return NextResponse.json({ 
        error: 'Erro ao buscar árvore de liderança', 
        details: treeErr instanceof Error ? treeErr.message : 'Erro desconhecido' 
      }, { status: 500 })
    }

    // Remove o próprio líder da lista
    const allowedIds = leadershipTree
      .map(node => node.id)
    ...
    if (error) {
      console.error('POST /api/admin/lideranca/rede-completa/presencas:', error)
    ...
    console.error('POST /api/admin/lideranca/rede-completa/presencas exception:', err)
