...
export default function AdminInstagramPostsPage() {
  ...
  function loadScheduled() {
    adminFetchJson<ScheduledItem[]>('/api/social/scheduled')
      ...
      .catch((err) => {
        console.error('[loadScheduled] falha ao buscar postagens programadas:', err)
      })
  }
    ...
    <PageAccessGuard pageKey="instagram">
      <div className="space-y-6 p-4 md:p-8">
        ...
        <div className="flex rounded-xl border border-slate-200 bg-white p-1 shadow-sm">
          {([
            { key: 'calendar', icon: CalendarDays, label: 'Calendário', count: scheduledItems.length + legacyItems.length },
            { key: 'list',     icon: List,         label: 'Lista',       count: scheduledItems.length },
            { key: 'insights', icon: BarChart2,     label: 'Insights',    count: null },
            ...
            <button
              ...
              className={`flex flex-1 items-center justify-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${
                activeTab === key
                  ? 'bg-[#c62737] text-white shadow-sm'
