...
// Component
// ─────────────────────────────────────────────
export function PostCard({ post, thumbnailUrl, metrics, integrations, onDeleted }: PostCardProps) {
  ...
  // detail modal (abre com duplo-clique)
  ...
  async function handleDelete() {
    setDeleting(true)
    try {
      await adminFetchJson(`/api/social/scheduled/${post.id}`, { method: 'DELETE' })
  ...
  return (
    <article
      className="group flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all hover:shadow-md hover:border-slate-300"
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-0 p-4">
        ...
        <div className="mt-auto flex flex-col gap-2 pt-3 border-t border-slate-100">
          {/* Linha principal: ação primária + botão excluir */}
          <div className="flex gap-2">
            {/* Ver no Instagram — publicadas com media ID */}
            ...
            {(post.status === 'pending' || post.status === 'failed') && (
              <button
                ...
                className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-red-200 bg-red-50 text-red-500 transition-colors hover:bg-red-100 hover:text-red-700"
              >
          ...
          {/* Linha secundária: ver detalhes / refazer para publicadas */}
          {post.status === 'published' && (
            <Link
              ...
              className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-600 transition-colors hover:bg-slate-50"
            >
              <Pencil className="h-3.5 w-3.5" />
              Refazer postagem
            </Link>
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-600 transition-colors hover:border-[#c62737] hover:bg-[#c62737]/5 hover:text-[#c62737]"
          >
        ...
        <ConfirmDialog
          ...
          variant="danger"
          title="Excluir publicação"
          message={`A publicação "${title}" será removida da fila. O histórico será mantido no sistema. Deseja continuar?`}
