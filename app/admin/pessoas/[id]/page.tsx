...
export default function PessoaDetalhePage() {
  ...
  const [loadingInviteRoles, setLoadingInviteRoles] = useState(false)
  ...
  async function loadInviteRoles() {
    setLoadingInviteRoles(true)
    try {
      const data = await adminFetchJson<{ roles?: InviteRoleOption[] }>('/api/admin/roles')
      setInviteRoles((data.roles ?? []).filter((role) => role.is_active !== false))
    } catch {
      setInviteRoles([])
    } finally {
      setLoadingInviteRoles(false)
    }
  }
  ...
  async function handleSendInvite(e: React.FormEvent) {
    ...
    if (!id) return
    if (!inviteProfile) {
      setInviteMessage({ type: 'err', message: 'Selecione o perfil de acesso.' })
      return
    }
          ...
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl shadow-lg max-w-md w-full p-6">
              ...
              <form onSubmit={handleSendInvite} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-2">Perfil de acesso do usuário</label>
                  <select
                    value={inviteProfile}
                    onChange={(e) => setInviteProfile(e.target.value)}
                    required
                    disabled={loadingInviteRoles}
                    className="w-full px-4 py-3 rounded-xl border border-slate-300 focus:border-[#c62737] focus:ring-2 focus:ring-[#c62737]/20 outline-none transition-all"
                  >
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Defina o perfil de acesso que o usuário terá ao completar o cadastro.
                  </p>
                </div>
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    type="submit"
                    disabled={inviteSending || (!personEmail && !inviteEmail.trim()) || !inviteProfile || loadingInviteRoles}
