...
/**
 * Retorna a data em formato ISO (YYYY-MM-DD) para America/Maceio.
 */
export function getLocalDateMaceio(offsetDays = 0): string {
  // Pegamos a data ATUAL em Maceió primeiro
  const now = new Date();
  const maceioTime = new Intl.DateTimeFormat('fr-CA', {
  ...
  // maceioTime vem como "YYYY-MM-DD, HH:mm:ss" ou "YYYY-MM-DD HH:mm:ss"
  const datePart = maceioTime.split(/[ ,]/)[0];
  const [y, m, d] = datePart.split('-').map(Number);
  
  // Agora criamos um objeto de data que GARANTE ser meio-dia daquele dia em Maceió
  // para evitar que o offset jogue para o dia anterior/seguinte por causa de horas.
...
function getDiaSemana(dateStr: string): string {
  ...
  const dt = new Date(y, m - 1, d, 12, 0, 0) // Meio-dia para evitar mudança de dia por fuso
...
// ── Preview utility for the admin panel ───────────────────────────────────────

/**
 * Interface para exibição no frontend de monitoramento.
 */
export interface ProximoDisparo {
  person_id: string
  name: string
  event_id: string
  event_name: string
...
/**
 * Retorna a lista de próximos disparos previstos (D3, D1, D0) com base na 
 * data atual e nas escalas publicadas.
 */
export async function getProximosLembretes(supabase: SupabaseClient): Promise<ProximoDisparo[]> {
  const result: ProximoDisparo[] = []

  // Buscamos as 3 datas alvo: hoje (D0), amanhã+2 (D3) e amanhã (D1)
  const targets: { kind: TemplateKind, date: string }[] = [
    { kind: 'D0', date: getLocalDateMaceio(0) },
    { kind: 'D1', date: getLocalDateMaceio(1) },
    { kind: 'D3', date: getLocalDateMaceio(3) },
  ]

  for (const { kind, date } of targets) {
    const assignments = await fetchAssignmentsByDate(supabase, date)
    if (!assignments.length) continue
    ...
    for (const item of assignments) {
      ...
      if (isSent) {
        try {
          // Tentamos buscar o log específico para ver se falhou
          ...
          status = logEntry?.status === 'failed' ? 'failed' : 'sent'
        } catch (e) {
          console.error(`[escalas-lembretes] erro ao buscar log para ${key}:`, e)
          // Se falhar a busca do status mas sabemos que está 'blocked' (isSent), assumimos sent por segurança
          status = 'sent'
        }
      }
      ...
      result.push({
        ...
        event_date: item.slotDate,
        event_time: item.slotTime,
        template_kind: kind,
        scheduled_for: scheduledForStr,
        status,
        message_preview: `Olá ${nomeExib}, lembrete de ${item.funcoes.join(', ')}...`
      })
    }
  }

  // Ordena por tipo de lembrete (D0 primeiro) e depois por voluntário
  const order = { D0: 0, D1: 1, D3: 2 }
  return result.sort((a, b) => {
    if (order[a.template_kind] !== order[b.template_kind]) {
      return (order[a.template_kind] as number) - (order[b.template_kind] as number)
    }
